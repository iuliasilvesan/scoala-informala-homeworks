...
/**
 * Creates Motorcycle objects which have the following characteristics: A
 * motorcycle has a fuel tank of a predefined size. A motorcycle has a fuel type
 * of a predefined type(e.g PETROL, DIESEL, HYBRID, ELECTRIC). A motorcycle has
 * a given number of gears. The maximum is 6. A motorcycle has a predefined
 * consumption of fuel per 100kms. A motorcycle has a predefined pollution per
 * km. The following values will be configured from outside of the class: A
 * motorcycle must be set up with a current fuel amount. A motorcycle must be
 * set up with a chassis number. Class Motorcycle implements interface Vehicle
 * and it adds to it behavior the following: The gear can be changed. The
 * consumption of fuel for a given number of kilometers in a given gear will be
 * implemented by all the children.
 */

abstract public class Motorcycle implements Vehicle {
	/** size of fuel tank */
	 ...
	 */
	Motorcycle(float availableFuel, String chassisNumber, float fuelTank, String fuelType, int numberOfGears,
	...
	/**
	 * Method designed to start the motorcycle
	 */
	...
	public void start() throws IllegalArgumentException {
		...
		if (this.getCurrentGear() > 1) {
			throw new IllegalArgumentException("Bad gear for starting the motorcycle!");
		}
	}
	...
	/**
	 * Method designed for stopping the motorcycle Will obtain the consumption
	 * and pollution when motorcycle is stopped.
	 */
	public void stop() {
